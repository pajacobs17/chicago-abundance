...
All the placeholder content — committee name, mission text, and
priorities — lives in `index.html` and `issues.html`. Colors and
fonts are set as CSS variables at the top of `styles.css` (in
`:root`), so you can re-theme the whole site by editing a handful of
values there.
